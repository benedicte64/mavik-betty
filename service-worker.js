const CACHE='jarvis-gentlecare-v29';
const CORE=['./','./index.html','./planning.html','./employe.html','./admin.html','./gestion.html','./clients.html','./stock.html','./devis.html','./ordres.html','./vehicule.html','./direction.html','./gcos-comms.js','./jarvis-core.js','./mavik-insights.js','./icon.svg','./manifest.webmanifest','./storage.js','./install.js','./boot.js','./jarvis-responsive.css','./atelier-responsive.css'];
  ...
  if(nav){
    event.respondWith(fetch(event.request,{cache:'no-store'}).then(async response=>{
      ...
      if(url.pathname.endsWith('/employe.html')&&!html.includes('atelier-responsive.css')){
        html=html.replace('</head>','<link rel="stylesheet" href="atelier-responsive.css?v=29"></head>');
      }
      ...
      if(!html.includes('mavik-insights.js'))html=html.replace('</body>','<script src="mavik-insights.js?v=29"></script></body>');
      if(!html.includes('gcos-comms.js'))html=html.replace('</body>','<script src="gcos-comms.js?v=29"></script></body>');
      if(!html.includes('jarvis-core.js'))html=html.replace('</body>','<script src="jarvis-core.js?v=29"></script></body>');
