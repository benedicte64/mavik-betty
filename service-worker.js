...
self.addEventListener('fetch',event=>{
  ...
  if(nav){
    event.respondWith(fetch(event.request,{cache:'no-store'}).then(async response=>{
      ...
      html=html.replace(/GCOS v1\.0/g,'MAVIK GCOS v0.30.0');
      if(!html.includes('mavik-insights.js'))html=html.replace('</body>','<script src="mavik-insights.js?v=30"></script></body>');
